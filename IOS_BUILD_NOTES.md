...
| Icon package migrated off `phosphor_flutter` | `pubspec.yaml` → `phosphor_icons: ^3.0.1`; the import swapped in **56 files** | Flutter 3.43 made `IconData` a `final class`. `phosphor_flutter` extends it, its last release is 2.1.0 (May 2024) and it is unmaintained, so builds 9 and 10 both died at `kernel_snapshot_program` with *"The class 'IconData' can't be extended outside of its library"*. `phosphor_icons` is the maintained continuation of the same package — same icons, same code points, same `PhosphorIconsStyle` enum, same `PhosphorIcon extends Icon` widget — and it fixes the break with `typedef PhosphorIconData = IconData`. Only the import line changed. **Note `flutter analyze` never caught this**: it analyses your `lib/` and not `.pub-cache`, so the project analysed clean while it could not compile. The check that matters is `flutter build bundle`, which now exits 0 locally. |
