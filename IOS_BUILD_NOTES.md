...
| O8 | **`phosphor_flutter` pins the whole toolchain.** Its latest release is 2.1.0 (May 2024) and it is unmaintained. Newer Flutter made `IconData` a `final class`, which 2.1.0 extends, so `flutter: stable` fails at `kernel_snapshot_program` with *"The class 'IconData' can't be extended outside of its library"*. `codemagic.yaml` is pinned to `flutter: 3.44.8` because of this. | You, eventually | The app cannot take a Flutter upgrade — including security and App Store compatibility fixes — until the package is replaced. It is used in **56 files, 375 references**, so this is a planned piece of work, not a quick swap. Do not set `flutter:` back to `stable`; build 9 died exactly there. |
