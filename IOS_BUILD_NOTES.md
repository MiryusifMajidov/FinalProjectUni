...
|---|---|---|
| O1 | **`ios/Runner/GoogleService-Info.plist` is not in the repo.** Confirmed absent. | Core Firebase still initialises from `lib/firebase_options.dart`, so the app runs — but Google Sign-In has no iOS OAuth client and fails on device. |
| O2 | **`Info.plist` still contains the literal placeholder** `REPLACE_WITH_REVERSED_CLIENT_ID` in `CFBundleURLSchemes`. | Google Sign-In cannot complete its callback. A reviewer tapping the Google button sees a broken feature, which is Guideline 2.1. Must be replaced with the real `REVERSED_CLIENT_ID` from `GoogleService-Info.plist`. |
| O3 | **[VERIFY] Sign in with Apple entitlement.** `pubspec.yaml` now carries `sign_in_with_apple: ^6.1.4` and `lib/core/services/auth_service.dart` implements the nonce, `SignInWithApple.getAppleIDCredential` and `completeAppleSignUp` — but at the time of writing `Runner.entitlements` contains only `aps-environment`. | The plugin throws at runtime without `com.apple.developer.applesignin` in the entitlements file. Check the file before building; another agent owns it. |
| O4 | **`ios/Podfile.lock` does not exist.** It cannot be generated on Windows. | Pod versions float between builds, so a build that was green yesterday can break today with no repo change. Fixed by section 5. |
| O5 | **[VERIFY] No in-app report-user / report-content flow.** Only a *report a bug* feedback form exists. | Guideline 1.2 requires reporting **and** blocking. Block exists; reporting does not. See `APP_STORE_METADATA.md` section 10. |
| O6 | **[VERIFY] `firebase.json` has no `hosting` block**, so `https://chess-ac4eb.web.app/support` and `/privacy` will not resolve. | Apple fetches the privacy URL during review. A 404 is an automatic 5.1.1 rejection. |
| O7 | **[VERIFY] In-app privacy text is stale** — `lib/features/settings/screens/privacy_screen.dart` names the app "Grandmaster" and gives `privacy@grandmasterapp.com`. | Contradicts the store listing, the hosted policy and the App Privacy label. |
| O8 | Codemagic UI one-time setup (steps 6 and 7 below) has not been done. | No build can start at all. |
